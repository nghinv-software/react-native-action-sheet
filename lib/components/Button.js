...
export default function Button(props?: ButtonActionSheetProps) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      onPress={onPress}
      style={[styles.container, isNullOrUndefined(checked) && !leftIconName && !renderRight && { justifyContent: 'center' }]}
    >
      {
        leftIconName && (
          <Icon
      ...
      <Text
        ...
        numberOfLines={2}
        style={[styles.txtTitle, { color: titleColor }, (!isNullOrUndefined(checked) || renderRight) && { flex: 1 }]}
      >
        {title}
      </Text>
